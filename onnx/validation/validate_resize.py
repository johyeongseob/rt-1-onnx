...
"""Save the NumPy RT-1 image resize output for one frame."""
...
from film_efficientnet import preprossors  # pylint: disable=g-import-not-at-top


def _parse_args() -> argparse.Namespace:
  parser = argparse.ArgumentParser(
      description="Run the NumPy resize used before RT-1 EfficientNet-B3."
  )
...
def main() -> None:
  ...
  converted = preprossors.convert_dtype_and_crop_images(image_batch)
  output = preprossors.resize_images(converted)

  artifact_dir = args.output_dir / episode_name / "resize"
  artifact_dir.mkdir(parents=True, exist_ok=True)
  output_path = artifact_dir / "onnx.npy"
